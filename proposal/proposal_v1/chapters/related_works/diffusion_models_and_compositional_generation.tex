...
Their work revealed that compositionality need not rely on explicit latent variables or conditional encoders; instead, it could emerge from the algebra of probability distributions governed by energy functions.  
...
By viewing diffusion models as energy-based systems, they define compositional operators—logical conjunction (AND) and negation (NOT)—that allow for flexible test-time control without additional training.  
...
More recently, \citet{skreta2024superposition} propose a novel framework for combining pretrained diffusion models under the principle of \emph{superposition}.  
...
At the same time, Bayes- or product-style composition over densities or scores should not be conflated with genuine out-of-distribution semantic composition: algebraic combination defines a mathematical operator, but it does not by itself guarantee that the resulting samples correspond to meaningful unseen concepts \citep{du2024compositional}.  As emphasized by \citet{du2023reduce,du2024compositional}, successful composition also depends on the compatibility of the underlying representation, since misaligned or entangled factors can cause composed models to merge conflicting features rather than coherent semantics.
