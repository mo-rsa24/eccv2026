"""
Export SD-IPC reachability records for the CO3 branch of the Phase 1 taxonomy study.

For each taxonomy pair-seed record that has a co3.png, this script:

1. Loads the stored CO3 image from `taxonomy_qualitative/`
...
4. Reruns the original CO3 condition from the same initial noise while tracking the CO3 path
   (CO3 itself is endpoint-only, so the "CO3 path" here is the shared PoE baseline rerun
   used only to provide a reference anchor for d_T comparison)
5. Measures d_t^{sdipc->co3}, d_T^{sdipc->co3}, and CLIP image similarity to the
   original CO3 image

The resulting JSON can be passed into:
    scripts/plot_phase1_reachability_qualitative.py --source co3 --reachability-json <path>
"""
...
def build_record(summary_path: Path) -> dict[str, Any] | None:
    ...
    co3_image_path = run_dir / "co3.png"
    if not co3_image_path.exists():
        return None
...
def compute_reachability_record(
    ...
    *,
    ...
    # Re-run PoE from the same seed to use as the trajectory reference anchor.
    # This gives us a consistent reference point aligned with the PoE reachability study.
    ...
    co3_source = Image.open(record["co3_image_path"]).convert("RGB")
    proj_vec = sdipc_project(co3_source, clip_model, clip_processor, inv_text, visual_proj, device)
    ...
    # Compare the SD-IPC rerun trajectory against the PoE reference trajectory.
    # CO3 has no denoising trajectory of its own (endpoint-only), so the PoE
    # trajectory serves as a shared-noise reference for d_t / d_T measurements.
    ...
    poe_rerun_image = decode_to_pil(models["vae"], poe_final)
    ...
    if image_root is not None:
        ...
        sdipc_path    = seed_dir / "co3_sdipc.png"
        poe_rerun_path = seed_dir / "poe_rerun.png"
        sdipc_image.save(sdipc_path)
        poe_rerun_image.save(poe_rerun_path)
        sdipc_image_path    = str(sdipc_path)
        poe_rerun_image_path = str(poe_rerun_path)

    source_emb      = clip_image_embedding(co3_source, clip_model, clip_processor, device)
    regen_emb       = clip_image_embedding(sdipc_image, clip_model, clip_processor, device)
    poe_anchor_emb  = clip_image_embedding(poe_rerun_image, clip_model, clip_processor, device)
    ...
    return {
        ...
        "co3_image_path": str(record["co3_image_path"]),
        "co3_sdipc_image_path": sdipc_image_path,
        "poe_rerun_image_path": poe_rerun_image_path,
        "d_t_sdipc_co3": [float(v) for v in d_t],
        "d_T_sdipc_co3": d_T,
        "clip_cosine_sdipc_co3": clip_cos,
        "cycle_close_rate_sdipc_co3": cycle_close,
        "cycle_broken": bool(cycle_close < 0.5),
        "clip_cosine_poe_rerun_vs_co3": poe_anchor_cos,
    }


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export SD-IPC reachability records for the CO3 branch of the Phase 1 taxonomy study."
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output JSON path (default: experiments/eccv2026/reachability/phase1_co3_reachability.json).",
    )
    parser.add_argument(
        "--image-dir",
        type=str,
        default="",
        help="Directory for exported CO3 SD-IPC images (default: sibling phase1_co3_images/).",
    )
    parser.add_argument(
        "--groups",
        nargs="+",
    ...
    output_json = Path(args.output_json) if args.output_json else (
        PROJECT_ROOT / "experiments" / "eccv2026" / "reachability" / "phase1_co3_reachability.json"
    )
    image_dir = Path(args.image_dir) if args.image_dir else (
        output_json.parent / "phase1_co3_images"
    )
    ...
    if not all_records:
        print(
            "ERROR: no taxonomy pair-seed records with co3.png found for CO3 reachability export.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(f"Wrote CO3 SD-IPC reachability records → {output_json}")
